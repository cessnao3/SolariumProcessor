...
mod tests {
    ...
    fn basic_test() {
        ...
        assert!(assemble_ok[0].get() == 0x200);
        assert!(assemble_ok[1].get() == 0x43);
    }
    ...
    fn test_noop() {
        ...
        assert!(binary.len() == 2);
        assert!(binary[0].get() == 0u16);
        assert!(binary[1].get() == 0u16);
    }

    #[test]
    fn test_all_no_args() {
        let line_test: Vec<(&str, u16)> = vec![
            ("noop", 0),
            ("inton", 1),
            ("intoff", 2),
            ("reset", 3),
        ...
        for (arg, opcode) in args_to_test {
            ...
            for reg in 0..NUM_REGISTERS {
                ...
                for bin_val in binary {
                    assert!(bin_val.get() == expected_result);
                }
            }
        }
    }
    ...
    fn test_infinite_counter_program() {
        ...
        for (index, word) in binary.iter().enumerate() {
            ...
            if resulting_val != word.get() {
                println!("Location {index:x} does not match - expected {resulting_val:x} != {:x}", word.get())
            }

            assert_eq!(resulting_val, word.get())
        }
    }
    ...
    fn test_load_text() {
        ...
        for i in 0..data.len() {
            assert_eq!(data[i].get(), expected_output[i]);
        }
    }
