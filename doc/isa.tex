...
The stack pointer provides an offset from the stack pointer starting location. The base of the stack is defined by the memory location in the stack base register. If the stack pointer is ever popped such that its value would underflow 0 or would reach greater than the maximum allowed size, the processor will error and halt.
...
All available instructions are listed in Table \ref{table:instruction-table}. Note that any invalid instruction that is not provided in the table below results in an immediate halt of the processor. Note that, in the below logic, any indication where PC is incremented indicates that the standard \texttt{PC += 1} to move to the next instruction will be replaced by the logic provided in the description field. \todo{combine left/right shift?} \todo{add load reg = base + offset?} \todo{keep bxor?}
...
\begin{table}[h!]
    ...
    \begin{footnotesize}
        \begin{tabular}{cccc|c|l}
            ...
            0 & 0 & 0 & 3 & \texttt{ari} & Set Signed Arithmetic \\
            0 & 0 & 0 & 4 & \texttt{aru} & Set Unsigned Arithmetic \\
            0 & 0 & 0 & 5 & \texttt{reset} & \texttt{PC = Reset Vector}, \texttt{R[0-15] = 0} \\
            0 & 0 & 0 & 6 & \texttt{pop} & \texttt{--SP} \\
            0 & 0 & 0 & 7 & \texttt{ret} & \texttt{x = R[ret]} \\
            {} & {} & {} & {} & {} & $\forall_{i \in [15 \rightarrow 0]}$ \texttt{R[i] = mem[--SP]}, \texttt{++PC} \\
            {} & {} & {} & {} & {} & \texttt{R[ret] = x} \\
            0 & 0 & 0 & 8 & \texttt{retint} & $\forall_{i \in [15 \rightarrow 0]}$ \texttt{R[i] = mem[--SP]}, \texttt{++PC} \\
            0 & 0 & 0 & 9 & \texttt{halt} & Stop Program Execution \\
            ...
            8 & R[b] & R[a] & R[dst] & \texttt{rem [dst], [a], [b]} & \texttt{R[dst] = R[a] \% R[b]} \\
            ...
            12 & R[b] & R[a] & R[dst] & \texttt{bshft [dst], [a], [b]} & \texttt{R[dst] = R[a] << R[b]} (Negative shifts right) \\
            \hline
        \end{tabular}
    \end{footnotesize}
...
\begin{table}[h!]
    ...
    \begin{tabular}{l|cl}
        ...
        \texttt{\$spb} & 3 & Stack Pointer Base \\
        \texttt{\$ret} & 4 & Return \\
