...
\begin{table}[h!]
	...
	\begin{footnotesize}
		\begin{tabular}{cccc|c|l}
			...
			0 & 0 & 0 & 6 & \texttt{ret} & \texttt{$\forall_{i \in \{15:0\}}$ R[i] = mem[--SP]}, \texttt{PC += 1} \\
			...
			0 & 6 & R[cmp] & R[a] & \texttt{jz [cmp], [a]} & \texttt{PC = R[a] IF R[cmp] == 0} \\
			0 & 7 & R[cmp] & R[a] & \texttt{jzr [cmp], [a]} & \texttt{PC += R[a] IF R[cmp] == 0} \\
			0 & 8 & R[cmp] & R[a] & \texttt{jgz [cmp], [a]} & \texttt{PC = R[a] IF R[cmp] > 0} \\
			0 & 9 & R[cmp] & R[a] & \texttt{jgzr [cmp], [a]} & \texttt{PC += R[a] IF R[cmp] > 0} \\
			0 & 10 & R[args] & R[loc] & \texttt{call [loc], [args]} & \texttt{$\forall_{i \in \{0:15\}}$ mem[SP++] = R[i]}, \texttt{mem[SP++] = R[args]}, \texttt{PC = R[loc]} \\
			...
			9 & R[b] & R[a] & R[dst] & \texttt{bsft [dst], [a], [b]} & \texttt{R[dst] = R[a] << R[b]} \\
			\hline
		\end{tabular}
	\end{footnotesize}
