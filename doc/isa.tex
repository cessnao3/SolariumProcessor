...
\begin{table}[h!]
    ...
    \begin{tabular}{l|cccc}
        ...
        \hline
    \end{tabular}
    \caption{The Sol32 instruction format typically has one opcode and three possible arguments associated with a particular opcode}
...
\begin{table}[h!]
	...
	\begin{footnotesize}
		...
		\end{tabular}
	\end{footnotesize}
	\caption{Available instruction list for the Sol32 provides a variety of commands.}
...
Sol32 computers, by default, utilize the following character map. This is similar to the American Standard Code for Information Interchange (ASCII) format. The Sol32 character map is defined in Table \ref{table:sproc-character-map}. Any undefined entries in the character map are considered invalid characters.
...
\begin{table}[h!]
    ...
    \begin{tabular}{cc|cc|cc|cc}
        ...
        \hline
    \end{tabular}
    \caption{Sol32 character mapping}
...
\begin{table}[h!]
    ...
    \begin{tabular}{l|lll}
        ...
        \hline
        \texttt{0} & u8 & Read & Provides the current input queue size \\
        \texttt{1} & u8 & Read & Pops and provides a word off the front of the input queue \\
        & & & If no data is available, will return \texttt{0} by default \\
        \texttt{2} & u8 & Read & Provides the current output queue size \\
        \texttt{3} & u8 & Write & Pushes a word onto the output queue \\
        \texttt{4} & u8 & Write & Clears the input queue if the value written is nonzero \\
        \texttt{5} & u8 & Write & Clears the output queue if the value written is nonzero \\
        \hline
    \end{tabular}
...
\begin{table}[h!]
	...
	\begin{tabular}{l|lll}
		...
		Offset & Type & Read/Write & Usage \\
		\hline
		\texttt{0} & u32 & Read/Write & Gets/sets the clock interval in CPU cycles. Set to 0 to disable. \\
		\texttt{4} & u32 & Read & Provides the current clock counter in CPU cycles. \\
		{} & {} & {} & Will be between 0 and the above interval. \\
		\texttt{8} & u32 & Read/Write & The hardware IRQ to trigger. \\
		\hline
	\end{tabular}
	\caption{Serial Input and Output device provides a simple data structure to read and write data streams}
...
The following list some simple example programs that can be run on the Sol32.
...
\lstinputlisting[caption={Limited counter program}, label={listing:example-counter}]{../examples/spa/counter.smc}
...
\lstinputlisting[caption={Infinite counter program with register reference names}, label={listing:example-infinite-counter}]{../examples/spa/infinite_counter.smc}

\subsection{Hello World}

Listing  \ref{listing:hello-world} provides a program that will infinitely write "hello, world" to the serial device, expected to be memory-mapped into the Sol32 at \texttt{0xA000}. This makes use of the \texttt{.loadtext} command, which loads the text, and an additional null-terminator at the end, directly into memory. It also makes use of a print-string function call to handle printing string values to the serial output devices.

\lstinputlisting[caption={Infinite "hello, world!" program}, label={listing:hello-world}]{../examples/spa/hello_world.smc}
...
\lstinputlisting[caption={Serial echo program reads in text characters and immediately outputs via the output device}, label={listing:serial-echo}]{../examples/spa/serial_echo.smc}
...
Several tools can help in the development of Sol32 programs.

\subsection{V/SProc}

One useful tool is \texttt{V/SProc}, \comment{which is a program that}{Update image!} combines together a basic assembler, CPU emulator, and memory inspector into a single program. The main window can be seen in Figure \ref{fig:visual-sproc-main-page}.
