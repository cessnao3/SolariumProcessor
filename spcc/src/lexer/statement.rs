...
fn read_variable_def(
    ...
    if let Some(Token::Symbol(Symbol::OpenBracket)) = iter.peek() {
        ...
        match iter.next() {
            ...
            Some(tok) => {
                return Err(format!(
                    "size must be provided as a word literal, found {0:}",
                    tok.to_string()
                ))
            }
    ...
    if let Some(Token::VariableName(name)) = iter.next() {
        ...
        match variable_type {
            VariableType::Static => {
                let variable_label = format!(
                    "static_variable_{0:}_{1:}",
                    variable_name,
                ...
                variable_value = Rc::new(StaticVariable::new(&variable_name, &variable_label));
            }
            VariableType::Stack => {
                let variable_offset = scopes.get_stack_offset();
                scopes.add_stack_offset(variable_size);

                variable_value = Rc::new(Variable::new(&variable_name, variable_offset as i32));
    ...
    if let Some(Token::Symbol(Symbol::Assignment)) = next_val {
        ...
        match read_base_expression(iter, scopes, REG_DEFAULT_PRIMARY_A, REG_DEFAULT_SPARE) {
            Ok(asm) => {
                ...
                vals.extend(
                    variable_value
                        .set_value_from_register(REG_DEFAULT_PRIMARY_A, REG_DEFAULT_SPARE),
                );
...
pub fn read_base_statement(
    iter: &mut TokenIter,
    scopes: &mut ScopeManager,
) -> Result<Option<ProgramSection>, String> {
    if let Some(tok) = iter.next() {
        ...
        match tok {
            ...
            Token::Keyword(Keyword::Func) => {
                ...
                let function_label =
                    format!("function_{0:}_{1:}", function_name, scopes.generate_index());
                ...
                for (i, name) in varnames.iter().enumerate() {
                    scopes.add_variable(
                        ...
                        Rc::new(Variable::new(
                            name,
                            (i as i32)
                                - (sproc::cpu::SolariumProcessor::NUM_REGISTERS as i32)
                                - (varnames.len() as i32),
                        )),
                    )?;
                }
            ...
            _ => {
                return Err(format!(
                    "unable to find base expression for token {0:}",
                    tok.to_string()
                ))
            }
        }
